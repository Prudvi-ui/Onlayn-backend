...
NotificationRoute.get("/notifications", userAuth, async (req, res) => {
  try {
    ...
    // Fetch only personal + global notifications (skip dashboard ones)
    const notifications = await notificationModel
  .find({
    $or: [
      // personal (any source, including dashboard)
      { customerId: customerId },

      // global (but exclude dashboard-only globals)
      { customerId: null, source: { $ne: "dashboard" } },
    ],
  })
  .sort({ createdAt: -1 });
