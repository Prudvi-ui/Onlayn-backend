...
DeliveryRoute.post("/create-delivery", adminAuth, async (req, res) => {
  try {
    const { orderId, agentId, orderStatus, address, deliveryAgent, estimatedDate, customerId ,mobileNumber } = req.body;

    // 1. Server-side Validation for required fields
    if (!orderId || !agentId || !address || !deliveryAgent || !estimatedDate || !customerId  || !mobileNumber) {
      return res.status(400).json({ success: false, message: 'All required fields must be provided.' });
    }
