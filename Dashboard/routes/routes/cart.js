...
//       productId,
//       productName: product.productName,
//       discountedPrice: discountedPrice,          // ✅ use discounted price
...
cartRoute.post("/create-cart", userAuth, async (req, res) => {
  try {
    ...
    const newCartItem = new cartModel({
      customerId,
      customerName: customerName || "Guest", // ✅ from frontend
      productId,
      productName: product.productName,
      ...
      size: product.size || "One Size",
      stock: product.stock || 0,
      mainPrice: product.price,  // ✅ added missing comma
      material: product.material || "N/A",
      color: product.color || "N/A",
      age: product.age || "0+",
      gst: product.gst || 0,
 ...
 */
cartRoute.delete("/delete-cart-item/:cartItemId", async (req, res) => {
  try {
    const { cartItemId } = req.params;
    const { customerId } = req.body; // get customerId from request body

    if (!customerId) return res.status(400).json({ error: "customerId is required" });
    if (!isValidObjectId(cartItemId)) return res.status(400).json({ error: "Invalid cart item ID" });
...
// Clear entire cart
cartRoute.delete("/clear-cart", async (req, res) => {
  try {
